...
规划设计提供右侧四视图面板及独立“迁移规划”工作台；规划/风险管理页支持与阶段会话共享的小对话。可补充业务属性、依赖、约束和容量，预览后应用批次调整，导出三 Sheet 输入模板与五 Sheet 计划（SpreadsheetML `.xls`）。默认 200 台，完整分配当前可纳入范围；1 万台通过分页处理。上传和日期排程均为模拟，不解析实际 Excel、不接真实模型或规划算法。
