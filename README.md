...
| `VITE_API_BASE_URL` | 空     | 预留给真实适配器的接口基础地址，当前不会发起请求       |
| `VITE_DEPLOY_BASE`  | `/`    | 静态部署路径，如 `/migration/`                         |

这些变量在构建时写入前端，**不可放密码、API Key 等秘密**。修改后重新启动开发服务或重新构建。Nexent 配置通过界面输入，本轮仅在内存保存并标记为未验证。
...
- [架构与接口适配](docs/design.md)：目录职责、状态流、契约和常见修改位置。
