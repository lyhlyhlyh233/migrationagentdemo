...
function applyPreference<K extends keyof Preferences>(
  key: K,
  value: Preferences[K],
) {
...
export function selectPreference<K extends keyof Preferences>(
  key: K,
  value: Preferences[K],
) {
  ...
  try {
    ...
    /* Keep settings usable without browser storage. */
  }
  ...
  function onStorage(event: StorageEvent) {
    for (const key of Object.keys(defaults) as (keyof Preferences)[]) {
      if (event.key === preferenceKeys[key] || event.key === null) {
        applyPreference(
          key,
          validPreference(key, event.newValue) ? event.newValue : defaults[key],
        );
      }
    }
    onChange();
