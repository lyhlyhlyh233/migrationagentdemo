...
import { ConversationAgentPicker, agentGuidance, conversationAgents, type ConversationAgentId } from './conversation-agent-picker';
import { ConversationAnswer, ThinkingIndicator, previewDiscussionReply, previewThoughtSummary, type ReplyPresentation } from './conversation-answer';
...
interface ConversationView { draft?: string; typing?: boolean; replyStartedAt?: number; workOpen?: boolean; panel?: PanelId; modelId?: ConversationModelId; agentId?: ConversationAgentId }
...
function WorkspaceSession({ onSignOut }: { onSignOut: () => void }) {
  ...
  return <>
    ...
    {[{ id: 'lobby', info: null }, ...projects].map((entry) => <div key={entry.id} hidden={creating || selected !== entry.id}>
      <ProjectWorkspace onOpenSettings={() => setSettingsOpen(true)} navCollapsed={navCollapsed} onNavCollapsedChange={setNavCollapsed} visible={!creating && selected === entry.id} project={entry.info} projects={projects} projectId={entry.id} onSelectProject={setSelected} onNewProject={() => setCreating(true)} />
    </div>)}
  </>;
}
...
function ProjectSetup({ onCreate, onCancel }: { onCreate: (project: ProjectInfo) => void; onCancel: () => void }) {
  ...
  return <main className="project-setup-page">
    <header><span><span className="huawei-symbol" role="img" aria-label={t("华为")} />MigrationDirector Plus</span><button onClick={onCancel}><Icon name="close" size={16} />{t("返回工作空间")}</button></header>
    <form className="project-setup" onSubmit={submit}>
      <Icon name="folder" size={28} /><h1>{t("新建迁移项目")}</h1><p>{t("先建立项目，再从调研评估开始推进交付。")}</p>
      <div className="project-form-grid">
        <label className="full-field"><span>{t("项目名称")}</span><input autoFocus required maxLength={60} value={draft.siteName} onChange={(e) => setDraft({ ...draft, siteName: e.target.value })} placeholder={t("例如：华东数据中心迁移")} /></label>
        <label><span>{t("行业")}</span><Select aria-label={t("行业")} value={draft.industry} onValueChange={(value) => setDraft({ ...draft, industry: value })}>{['金融', '运营商', '公安', '政府', '教育'].map((item) => <option key={item} value={item}>{t(item)}</option>)}</Select></label>
        <label><span>{t("迁移类型")}</span><Select aria-label={t("迁移类型")} value={draft.migrationType} onValueChange={(value) => setDraft({ ...draft, migrationType: value })}>{['虚拟化', 'SAN', 'NAS', '对象'].map((item) => <option key={item} value={item}>{t(item)}</option>)}</Select></label>
        <label><span>{t("地区部")}</span><Select aria-label={t("地区部")} value={draft.region} onValueChange={(value) => setDraft({ ...draft, region: value })}>{['中国地区部', '中亚地区部', '亚太地区部'].map((item) => <option key={item} value={item}>{t(item)}</option>)}</Select></label>
        <label><span>{t("代表处")}</span><input required maxLength={60} value={draft.office} onChange={(e) => setDraft({ ...draft, office: e.target.value })} placeholder={t("例如：上海代表处")} /></label>
      </div>
      <div className="setup-next"><Icon name="info" size={16} /><p>{t("创建后自动开启评估会话。任务、风险、交付件和操作记录都归属当前项目。")}</p></div>
      <div className="setup-actions"><button type="button" onClick={onCancel}>{t("取消")}</button><button className="primary" type="submit">{t("创建项目，开始评估")}<Icon name="right" size={16} /></button></div>
    </form>
  </main>;
}

function ProjectWorkspace({ onOpenSettings, navCollapsed, onNavCollapsedChange, visible, project, projects, projectId, onSelectProject, onNewProject }: { onOpenSettings: () => void; navCollapsed: boolean; onNavCollapsedChange: (collapsed: boolean) => void; visible: boolean; project: ProjectInfo | null; projects: ProjectEntry[]; projectId: string; onSelectProject: (id: string) => void; onNewProject: () => void }) {
  const t = useTranslation();
  ...
  function conversationTitle(chat?: StageConversation) { return chat ? !chat.manuallyNamed && (chat.kind === 'main' || chat.title === '新会话') ? t(chat.title) : chat.title : t('项目会话'); }
  ...
  const conversationAgent = view.agentId ?? (temporaryChat ? 'general' : activeStage);
  const conversationAgentLabel = conversationAgents.find((agent) => agent.id === conversationAgent)!.label;
  ...
  function setAgentTyping(typing: boolean) { updateConversationView({ typing, replyStartedAt: typing ? Date.now() : undefined }); }
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => project ? [
    { id: 1, role: 'system', text: t('项目「{0}」已创建，评估会话已开启。', project.siteName), time: now(), conversationId: mainConversationId('research'), stageId: 'research', operation: true },
    { id: 2, role: 'agent', text: t('我们从调研评估开始。请上传 RVTools 采集表和售前调用表，也可以先使用示例资料体验流程。\n\n我会核对资产清单与兼容性，把结果汇总到项目风险和交付件中。'), time: now(), conversationId: mainConversationId('research'), stageId: 'research' },
  ] : []);
  ...
  function newChat() {
    ...
    setTemporaryChats((items) => [...items, { id, title: t('新聊天') }]);
  ...
  useEffect(() => {
    const delivery = window.setTimeout(() => {
      ...
      (['creation', 'sync', 'cutover'] as ExecutionTaskKind[]).forEach((kind) => {
        ...
        setMessages((items) => [...items, { id: Date.now() + Math.random(), role: 'system', text: t(`${metric.total} 个${label}任务已完成。${kind === 'cutover' ? '配置对比已更新，可在结果验证阶段查看。' : ''}`), time: now(), ...origin, operation: true }]);
      });
  ...
  function appendMessage(role: ChatMessage['role'], text: string, target = conversationId, operation = role === 'system', stageOverride?: StageId, selection?: Pick<ChatMessage, 'modelId' | 'agentId' | 'reply'>) {
    ...
    setMessages((items) => [...items, { id: Date.now() + Math.random(), role, text: role === 'user' ? text : t(text), time: now(), conversationId: resolvedTarget, stageId: stageOverride || targetStage || (resolvedTarget.startsWith('chat-') ? undefined : activeStage), operation, ...selection }]);
  }
  ...
  function sendPrompt(text: string) {
    ...
    const requestSelection = { modelId: conversationModel, agentId: conversationAgent };
    ...
    appendMessage('user', text.trim(), target, false, undefined, requestSelection);
    setQuestion('');
    const startedAt = Date.now();
    updateConversationView({ typing: true, replyStartedAt: startedAt }, target);
    window.setTimeout(() => {
      let answer = '';
      if (temporaryChat) answer = previewDiscussionReply(text, requestSelection.agentId);
      else if (!project) answer = '先建立项目吧。填写局点、代表处和迁移类型后，我会帮你整理评估资料。';
      else if (requestSelection.agentId !== 'general' && requestSelection.agentId !== activeStage) answer = `${t(agentGuidance[requestSelection.agentId])}\n\n${t('当前会话属于{0}，执行操作仍按项目阶段和前置条件开放。', t(stageName[activeStage]))}`;
      ...
      } else if (/风险|兼容|risk|compatib/i.test(text)) answer = assessed ? `目前记录了 ${risks.length} 项风险，其中 ${highRiskOpen.length} 项高风险尚未闭环。\n\n优先检查 CPU 兼容性和业务性能基线。请在风险清单中记录处理措施与验证依据，完成高风险闭环后，我会引导你进入下一阶段。` : '我会重点核对 CPU 指令集兼容性、峰值 IOPS、网络与存储映射。评估完成后，每项风险都会标注等级和影响范围，高风险需要人工闭环。';
      else if (/资料|文件|采集|上传|file|upload|input/i.test(text)) answer = '调研评估需要 RVTools 采集表和售前调用表，支持 XLSX、XLS、CSV。前者用于核对资产和资源，后者用于确认业务基线与迁移约束。\n\n也可以直接使用示例资料；选择本地文件时只记录文件名，不会上传到服务器。';
      else if (/报告|产物|交付件|report|deliverable|download/i.test(text)) answer = assessed ? `评估报告已就绪，右侧“文件与产物”可以直接下载。${planned ? '批次计划和 RunBook 也已生成。' : '完成规划信息后，还会生成批次计划和 RunBook。'}所有文件都可以在左侧“迁移交付件”中查看。` : '评估完成后，我会生成风险清单与评估报告；规划完成后再输出批次计划和 RunBook。当前可以先确认输入资料并启动评估。';
      ...
      appendMessage('agent', answer, target, false, undefined, { ...requestSelection, reply: { summary: t(previewThoughtSummary(text, requestSelection.agentId)), durationMs: Date.now() - startedAt } });
      replyLocks.current.delete(target);
      updateConversationView({ typing: false, replyStartedAt: undefined }, target);
    }, 1400);
  }
  ...
  const quickGroups: Record<StageId, QuickGroup[]> = {
    research: [
      { label: '资料准备', icon: 'folder', options: [{ label: '评估需要哪些资料？', description: '了解采集表与售前信息', onClick: () => sendPrompt(t('评估需要哪些资料？')) }, { label: '查看评估资料', description: '确认或替换当前输入文件', onClick: showStageWork }] },
      { label: '风险与报告', icon: 'shield', options: [{ label: '有哪些兼容性风险？', description: '了解评估的重点检查项', onClick: () => sendPrompt(t('有哪些兼容性风险？')) }, open('查看交付风险', '查询并处理当前风险', 'risk'), { label: '有哪些报告可以下载？', description: '查看当前阶段产物', onClick: () => sendPrompt(t('有哪些报告可以下载？')) }] },
      { label: '项目概况', icon: 'chat', options: [{ label: '总结当前项目范围', description: '虚拟机数量与业务背景', onClick: () => sendPrompt(t('总结当前项目范围')) }, { label: '告诉我下一步该做什么', description: '梳理当前阶段的待办', onClick: () => sendPrompt(t('告诉我下一步该做什么')) }] },
    ],
    planning: [
      { label: '范围与依赖', icon: 'folder', options: [{ label: '查看规划工作台', description: '确认范围、填写资料或生成批次', onClick: showStageWork }, { label: '总结当前迁移范围', description: '查看需要迁移的资产', onClick: () => sendPrompt(t('总结当前迁移范围')) }, { label: '如何填写业务依赖？', description: '业务分级、集群与上下游关系', onClick: () => sendPrompt(t('如何填写业务依赖？')) }] },
      ...
      { label: '风险与产物', icon: 'file', options: [open('查看规划风险', '处理进入实施前的风险', 'risk'), { label: '有哪些规划产物？', description: '批次计划和 RunBook', onClick: () => sendPrompt(t('有哪些规划产物？')) }] },
    ],
    migration: [
      { label: '环境检查', icon: 'search', options: [{ label: '当前 MD 配置与进度', description: '连接状态和实施准备', onClick: () => sendPrompt(t('当前 MD 配置与进度')) }, { label: '查看实施工作台', description: '回到连接检查与任务确认', onClick: showStageWork }] },
      ...
      { label: '进展与报告', icon: 'file', options: [{ label: '总结当前实施进度', description: '查看各类任务的执行进展', onClick: () => sendPrompt(t('总结当前实施进度')) }, open('查看交付件', '查看评估报告与实施文档', 'deliverables')] },
    ],
    validation: [
      { label: '配置对比', icon: 'search', options: [open('查看验证结果', '检查 25 项源端与目标端配置', 'validation'), { label: '如何核对验证结果？', description: '了解对比范围与人工验收', onClick: () => sendPrompt(t('如何核对验证结果？')) }] },
      { label: '人工验收', icon: 'tasks', options: [open('查看待确认虚拟机', '逐台或批量确认迁移结果', 'validation'), { label: '当前验收进度如何？', description: '查看已确认与待确认数量', onClick: () => sendPrompt(t('当前验收进度如何？')) }] },
  ...
  const showFlowPreview = !project || assessmentStatus === 'idle' || assessmentStatus === 'ready';
  ...
  return (
    <main className={`workspace ${navOpen ? 'nav-open' : ''} ${navCollapsed ? 'nav-collapsed' : ''} ${showInspector ? '' : 'inspector-collapsed'} ${isManagement ? 'management-view' : ''}`}>
      <a href={`#conversation-${projectId}`} className="skip-link">{t("跳转到对话")}</a>
      {navOpen && <button className="nav-scrim" aria-label={t("关闭导航")} onClick={() => setNavOpen(false)} />}
      <aside className="nav-rail" aria-label={t("折叠导航")}>
        <button ref={expandNavButton} className="icon-button" aria-label={t("展开左侧菜单")} title={t("展开左侧菜单")} aria-expanded={false} aria-controls={`project-navigation-${projectId}`} onClick={() => toggleNavigation(false)}><Icon name="sidebar" size={19} /></button>
        <button className="icon-button" aria-label={t("新建项目")} title={t("新建项目")} onClick={startNewProject}><Icon name="plus" size={19} /></button>
        <button className="icon-button" aria-label={t("新建聊天")} title={t("新建聊天")} onClick={newChat}><Icon name="chat" size={19} /></button>
        <button className="icon-button nav-rail-account" aria-label={t("打开设置")} title={t("设置")} onClick={onOpenSettings}><Icon name="settings" size={19} /></button>
      </aside>
      <aside className="workspace-nav" id={`project-navigation-${projectId}`} aria-label={t("项目导航")}>
        <div className="nav-platform"><span className="huawei-symbol" role="img" aria-label={t("华为")} /><h1>MigrationDirector <span>Plus</span></h1><button ref={collapseNavButton} className="icon-button nav-collapse" aria-label={t("折叠左侧菜单")} title={t("折叠左侧菜单")} aria-expanded={true} aria-controls={`project-navigation-${projectId}`} onClick={() => toggleNavigation(true)}><Icon name="sidebar" size={17} /></button><button className="icon-button nav-close" aria-label={t("收起导航")} onClick={() => setNavOpen(false)}><Icon name="close" size={16} /></button></div>
        <div className="project-switcher-row"><div className="project-switcher"><Icon name="folder" size={18} /><Select aria-label={t("切换当前项目")} title={project ? projectName : t('工作空间')} value={projectId} onValueChange={onSelectProject}><option value="lobby">{t("工作空间")}</option>{projects.map((item) => <option key={item.id} value={item.id}>{item.info.siteName}</option>)}</Select></div><button className="icon-button" title={t("新建项目")} aria-label={t("新建项目")} onClick={startNewProject}><Icon name="plus" size={18} /></button><button className="icon-button" title={t("新建聊天")} aria-label={t("新建聊天")} onClick={newChat}><Icon name="chat" size={18} /></button></div>
        <div className="nav-tree-scroll">
          <nav className="primary-nav project-resources" aria-label={t("项目资料")}>
            {([{ id: 'tasks', label: '迁移任务', icon: 'tasks' }, { id: 'risk', label: '迁移风险', icon: 'shield' }, { id: 'deliverables', label: '迁移交付件', icon: 'file' }, { id: 'logs', label: '操作日志', icon: 'clock' }] as const).map((item) => <ProjectResourceButton key={item.id} label={item.label} icon={item.icon} selected={panel === item.id} disabled={!project} visible={visible} count={item.id === 'risk' && risks.some((risk) => !risk.closed) ? risks.filter((risk) => !risk.closed).length : undefined} onClick={() => setPanel(item.id)} />)}
          </nav>
        ...
        <div className="nav-bottom"><div className="user-profile" aria-label={t("当前用户")}><span><Icon name="user" size={17} /></span><div><strong>{t("当前用户")}</strong><small>{t("个人账户")}</small></div><button className="icon-button account-settings" aria-label={t("打开设置")} title={t("设置")} onClick={onOpenSettings}><Icon name="settings" size={18} /></button></div></div>
      </aside>
      <section className="conversation-workspace">
        <div className="mobile-workspace-bar"><button className="icon-button" aria-label={t("打开导航")} onClick={() => setNavOpen(true)}><Icon name="menu" /></button><span>{project ? projectName : t('工作空间')}</span></div>
        {showStageRail && showFlowPreview && <StageFlowPreview hasProject={Boolean(project)} ready={assessmentStatus === 'ready'} onOpenAssessment={() => selectStage('research')} />}
        {showStageRail && !showFlowPreview && <section className="progress-rail" aria-label={t("迁移项目进度")}><div className="progress-caption"><span>{t("项目流程")}</span><span><strong>{t(overallProgress)}%</strong> {t(` 本轮进度 ·${stages.filter((s) => s.status === 'completed').length} / 4 阶段完成`)}</span></div><ol>{stages.map((stage, index) => <li key={stage.id} className={`${project && stage.id === workflowStage ? 'current' : ''} ${stage.status === 'completed' ? 'done' : ''} ${runningStages[stage.id] ? 'is-processing' : ''} ${project && activeStage === stage.id ? 'viewed-stage' : ''}`}><button disabled={!availableStages[stage.id] && !readyStages[stage.id]} onClick={() => selectStage(stage.id)} aria-current={project && stage.id === activeStage ? 'step' : undefined}><span className="stage-sequence">{stage.status === 'completed' ? <Icon name="check" size={12} /> : index + 1}</span><span>{t(stage.title)}</span><small>{t(!availableStages[stage.id] && readyStages[stage.id] ? '待确认' : `${stage.progress}%`)}</small></button><div className="stage-track" role="progressbar" aria-label={t(`${stage.title}进度`)} aria-valuemin={0} aria-valuemax={100} aria-valuenow={stage.progress}><i style={{ width: `${stage.progress}%` }} /></div></li>)}</ol></section>}
        {!isManagement && (project || temporaryChat) && <div className="conversation-toolbar"><div className="conversation-context"><Icon name={temporaryChat ? 'chat' : 'agent'} size={15} /><span>{temporaryChat ? temporaryChats.find((chat) => chat.id === temporaryChat)?.title : project ? (currentStageChat.title === active.title ? t(active.title) : `${t(active.title)} / ${conversationTitle(currentStageChat)}`) : ''}</span></div>{project && !temporaryChat && !showInspector && <button className="show-execution-details" onClick={toggleInspector}><Icon name="tasks" size={14} />{t("查看执行详情")}</button>}</div>}
        <div className="conversation-scroll" ref={conversation} id={`conversation-${projectId}`} tabIndex={-1}>
          <div className={`conversation-content ${panel ? 'has-inline-panel' : ''}`}>
            ...
            {project && !temporaryChat && <div className="inline-work" ref={inlineWork} key={conversationId}>
              {panel ? <section className={isManagement ? 'management-surface' : 'inline-artifact'} aria-label={t(isManagement ? panelNames[panel] : `对话中的${panelNames[panel]}`)}>{!isManagement && <div className="inline-artifact-label"><span><Icon name="file" size={15} />{t(panelNames[panel])}</span><button onClick={() => setPanel(null)}><Icon name="close" size={14} />{t("收起")}</button></div>}
                {panel === 'deliverables' && <div className="project-records"><h2>{t("迁移交付件")}</h2><p>{t("各阶段生成的文件统一归档在这里。")}</p>{artifacts.filter((item) => 'href' in item).length ? artifacts.filter((item) => 'href' in item).map((item) => <a className="deliverable-row" key={item.label} href={'href' in item ? item.href : undefined} download={'download' in item ? item.download : undefined}><Icon name="file" /><span><strong>{t(item.label)}</strong><small>{t(item.meta)}</small></span><Icon name="download" size={17} /></a>) : <div className="records-empty">{t("还没有交付件。完成评估后，首份报告会出现在这里。")}</div>}</div>}
                {panel === 'logs' && <div className="project-records"><h2>{t("操作日志")}</h2><p>{t("当前项目的执行事件与人工操作记录。")}</p><ol className="operation-log">{messages.filter((message) => message.operation).map((message) => <li key={message.id}><time>{t(message.time)}</time><div><small>{t(message.stageId ? t(stageName[message.stageId]) : t('项目操作'))} · {t(conversationTitle(stageConversations.find((chat) => chat.id === message.conversationId)))}</small><p>{message.text}</p></div></li>)}</ol></div>}
              ...
              {currentStageChat.kind === 'child' && <div className="stage-work-heading"><span>{t("阶段操作 · 项目共享")}</span><button onClick={() => updateConversationView({ workOpen: false })}>{t("收起")}</button></div>}
            {project && activeStage === 'research' && assessmentStatus !== 'completed' && (
              <div className="upload-card">
                <div className="upload-card-copy"><small>ASSESSMENT INPUTS</small><h3><Icon name="file" size={16} />{t("上传调研评估资料")}</h3><p>{t("支持 XLSX、XLS、CSV，可使用示例资料。")}</p></div>
                <div className="upload-items">
                  <label className={files.rvtools ? 'uploaded' : ''}><input type="file" accept=".xlsx,.xls,.csv" disabled={assessmentStatus === 'running'} onChange={(event) => handleFile('rvtools', event)} /><span><Icon name="file" size={17} /></span><div><strong>{t("RVTools 采集表")}</strong><small>{t(files.rvtools || '支持 XLSX / XLS / CSV')}</small></div><em>{t(files.rvtools ? '已就绪 ✓' : '选择文件')}</em></label>
                  <label className={files.presales ? 'uploaded' : ''}><input type="file" accept=".xlsx,.xls,.csv" disabled={assessmentStatus === 'running'} onChange={(event) => handleFile('presales', event)} /><span><Icon name="file" size={17} /></span><div><strong>{t("售前调用表")}</strong><small>{t(files.presales || '支持 XLSX / XLS / CSV')}</small></div><em>{t(files.presales ? '已就绪 ✓' : '选择文件')}</em></label>
                </div>
                <button className="sample-files" disabled={assessmentStatus === 'running'} onClick={useDemoFiles}>{t("使用示例资料")}</button><button className="inline-assess" disabled={!canStartAssessment} onClick={startAssessment}>{t(assessmentStatus === 'running' ? '评估子智能体正在分析…' : '启动评估子智能体')}</button>
              </div>
            )}

            {assessmentStatus === 'completed' && activeStage === 'research' && (
              <div className="assessment-result">
                <span>!</span><div className="assessment-result-copy"><small>ASSESSMENT COMPLETED · STAGE GATE BLOCKED</small><h3>{t("调研评估已完成")}</h3><p>{t(`识别${risks.length} 项风险，评估报告已生成。`)}</p>{highRiskOpen.length > 0 && <div className="assessment-gate-warning"><b>{t(highRiskOpen.length)}</b><span><strong>{t("项高风险待闭环")}</strong><em>{t("高风险全部闭环后才能进入规划设计阶段，请点击查看风险完成处理。")}</em></span></div>}</div>
                <button className="risk-gate-button" onClick={() => setPanel('risk')}>{t("查看并闭环风险 →")}</button><a href={reportHref} download={`${projectName}-迁移调研评估报告.txt`}>{t("下载报告 ↓")}</a>
              </div>
            )}
            ...
            {project && activeStage === 'planning' && planningStatus !== 'locked' && (
              <div className="planning-workflow">
                <div className="planning-steps">
                  <span className={planningStatus !== 'scope-review' ? 'done' : 'active'}><i>{t(planningStatus !== 'scope-review' ? '✓' : '1')}</i>{t("确认迁移范围")}</span><b />
                  <span className={planningStatus === 'details-pending' || planningStatus === 'generating' ? 'active' : planningStatus === 'completed' ? 'done' : ''}><i>{t(planningStatus === 'completed' ? '✓' : '2')}</i>{t("补充规划信息")}</span><b />
                  <span className={planningStatus === 'completed' ? 'done' : planningStatus === 'generating' ? 'active' : ''}><i>{t(planningStatus === 'completed' ? '✓' : '3')}</i>{t("生成规划批次")}</span>
                </div>

                {planningStatus === 'scope-review' && <div className="planning-card scope-card"><div className="planning-card-copy"><small>STEP 1 · MIGRATION SCOPE</small><h3><Icon name="tasks" size={16} />{t("确认虚拟机总数与迁移范围")}</h3><p>{t("下载资产列表核对范围；如需调整，修改后重新上传。")}</p></div><div className="scope-stat"><strong>{t(vmCount)}</strong><span>{t("台虚拟机")}</span><small>{t("初始迁移范围")}</small></div><div className="planning-card-actions"><a href={vmScopeHref} download={`${projectName}-虚拟机范围列表.xls`}>{t("↓ 下载并查看范围列表")}</a><label className={scopeRevisionFile ? 'uploaded' : ''}><input type="file" accept=".xlsx,.xls,.csv" onChange={uploadVmScope} />{t(scopeRevisionFile ? `已刷新：${scopeRevisionFile}` : '↑ 上传调整后的范围')}</label><button onClick={confirmVmScope}>{t(`范围无误，确认${vmCount} 台 →`)}</button></div></div>}

                {(planningStatus === 'details-pending' || planningStatus === 'generating') && <div className="planning-card details-card"><div className="planning-card-copy"><small>STEP 2 · PLANNING WORKBOOK</small><h3><Icon name="file" size={16} />{t("补充虚拟机业务信息与迁移约束")}</h3><p>{t("填写模板中的业务信息、依赖关系和迁移约束，上传后生成批次计划。")}</p></div><div className="sheet-requirements"><div><span>01</span><p><strong>{t("虚拟机清单")}</strong><small>{t("业务系统名称、业务等级、集群类型、集群角色")}</small></p></div><div><span>02</span><p><strong>{t("业务依赖关系")}</strong><small>{t("上下游系统、依赖类型、端口及说明")}</small></p></div><div><span>03</span><p><strong>{t("迁移约束条件")}</strong><small>{t("迁移窗口、约束说明和回退要求")}</small></p></div></div><div className="planning-card-actions"><a href={planningTemplateHref} download={`${projectName}-迁移规划信息模板.xls`}>{t("↓ 下载三 Sheet 规划模板")}</a><label className={planningWorkbook ? 'uploaded' : ''}><input type="file" accept=".xlsx,.xls" disabled={planningStatus === 'generating'} onChange={uploadPlanningWorkbook} />{t(planningStatus === 'generating' ? '正在校验并生成计划…' : planningWorkbook ? `已上传：${planningWorkbook}` : '选择已填写的规划模板')}</label><button onClick={useDemoPlanning} disabled={planningStatus === 'generating'}>{t("使用示例规划信息")}</button></div></div>}

                {planningStatus === 'completed' && <div className="planning-result"><span>✓</span><div><small>PLANNING COMPLETED</small><h3>{t("迁移批次与实施计划已生成")}</h3><p>{t(`共生成${batchTasks.length} 项批次任务和 4 项规划风险；规划高风险闭环后可确认批次。`)}</p></div><div className="planning-downloads"><a href={batchPlanHref} download={`${projectName}-迁移批次与实施计划.xls`}><small>{t("实施批次计划")}</small><strong>{t("下载")}</strong><span>↓</span></a><a href={runBookHref} download={`${projectName}-RunBook.md`}><small>RunBook</small><strong>{t("下载")}</strong><span>↓</span></a></div><div className="planning-result-actions"><button onClick={() => setPanel('tasks')}>{t("查看批次任务")}</button></div></div>}
                {planningStatus === 'completed' && batchConfirmation !== 'confirmed' && <div className={`batch-confirm-card ${batchConfirmation}`}><span className="batch-confirm-icon">?</span><div><small>STAGE GATE · BATCH CONFIRMATION</small><h3>{t("请确认迁移任务批次是否需要调整")}</h3><p>{t("可在对话中调整批次、范围或排期，确认后进入实施。")}</p></div><div className="batch-confirm-actions"><button onClick={requestBatchAdjustment}>{t("需要调整，与 Agent 交互")}</button><button className="primary" onClick={confirmBatchesAndPrepareMigration}>{t("审核交接，进入迁移实施 →")}</button></div></div>}
              </div>
            )}
            {project && activeStage === 'migration' && batchConfirmation === 'confirmed' && mdStatus !== 'ready' && <div className="md-onboarding-card">
              <div className="md-onboarding-head"><span>MD</span><div><small>MIGRATIONDIRECTOR CONNECTION</small><h3><Icon name="agent" size={16} />{t("配置近端 MigrationDirector")}</h3><p>{t("在近端 MD 填写以下连接信息，然后检查配置。")}</p></div><em className={`md-state state-${mdStatus}`}>{t(mdStatusText[mdStatus])}</em></div>
              <div className="md-connection-info"><div><small>{t("服务端 IP")}</small><strong>23.45.2.2</strong></div><div><small>{t("端口号")}</small><strong>7839</strong></div><div><small>{t("项目 ID")}</small><strong>x3ddrnb</strong></div></div>
              <div className="md-check-list">
                <div className={mdStatus !== 'unconfigured' ? 'done' : ''}><i>{t(mdStatus !== 'unconfigured' ? '✓' : '1')}</i><span><strong>{t("近端 MD 连接")}</strong><small>{t("验证服务端地址、端口与项目 ID")}</small></span></div>
                <div className={mdStatus === 'checking-config' ? 'done' : mdStatus === 'connected' ? 'active' : ''}><i>2</i><span><strong>{t("源端 VMware 发现配置")}</strong><small>{t("检查保护源与资源发现状态")}</small></span></div>
                <div className={mdStatus === 'checking-config' ? 'done' : ''}><i>3</i><span><strong>{t("目标端 FusionCompute 发现配置")}</strong><small>{t("检查目标资源池与保护配置")}</small></span></div>
                <div className={mdStatus === 'checking-config' ? 'done' : ''}><i>4</i><span><strong>{t("端口组映射")}</strong><small>{t("校验源端与目标端网络映射")}</small></span></div>
              </div>
              <div className="md-onboarding-actions"><span>{t("检查连接、资源发现和网络映射。")}</span><button onClick={startMdCheck} disabled={mdStatus !== 'unconfigured'}>{t(mdStatus === 'checking-connection' ? '正在检查连接…' : mdStatus === 'checking-config' ? '正在检查配置…' : '检查连接与配置 →')}</button></div>
            </div>}
            {project && activeStage === 'migration' && mdStatus === 'ready' && <>
              <details className="md-history-card"><summary><span><Icon name="check" size={15} />{t("MD 配置检查通过")}</span><span>{t("查看检查记录")}<Icon name="chevron" size={13} /></span></summary><div className="md-history-list">{mdHistory.map((item) => <div key={item.id}><time>{t(item.time)}</time><i>✓</i><p><strong>{t(item.title)}</strong><small>{t(item.detail)}</small></p></div>)}</div></details>
              {!allExecutionApproved && <div className="execution-approval-card"><div className="approval-card-head"><span>?</span><div><small>AGENT EXECUTION APPROVAL</small><h3>{t("三类任务可分别查看、独立确认")}</h3><p>{t("任意一类任务确认后立即启动，无需等待另外两类任务确认。")}</p></div></div><div className="execution-approval-list">{([{ kind: 'creation', label: '待新建任务', total: executionMetrics.creation.total }, { kind: 'cutover', label: '待割接任务', total: executionMetrics.cutover.total }, { kind: 'sync', label: '待增量同步任务', total: executionMetrics.sync.total }] as const).map((item, index) => <div className={executionApprovals[item.kind] ? 'confirmed' : 'current'} key={item.kind}><span>{t(executionApprovals[item.kind] ? '✓' : index + 1)}</span><p><strong>{t(item.label)}</strong><small>{t(`共${item.total} 个任务`)}</small></p><div className="approval-actions"><button className="view" onClick={() => openExecutionTaskPanel(item.kind)}>{t("查看任务")}</button><button disabled={executionApprovals[item.kind] || executionMetrics[item.kind].completed === item.total} onClick={() => confirmExecutionType(item.kind)}>{t(executionMetrics[item.kind].completed === item.total ? '已完成' : executionApprovals[item.kind] ? '正在执行' : '确认并立即执行')}</button></div></div>)}</div></div>}
              {allExecutionApproved && <div className="execution-summary"><Icon name="check" /><div><strong>{t("三类实施任务已确认")}</strong><p>{t("执行进度会持续更新在右侧。你可以继续查看任务明细。")}</p></div><button onClick={() => setPanel('tasks')}>{t("查看任务")}</button></div>}

            </>}
            {project && activeStage === 'validation' && validationTasks.length > 0 && <div className="validation-dashboard">
              <div className="validation-dashboard-icon">✓</div><div><small>RESULT VALIDATION · TODAY</small><h3>{t("割接已完成，验证结果列表已生成")}</h3><p>{t(`验证子智能体已对${validationTasks.length} 台虚拟机完成源端与目标端配置对比。请查看今日验证列表，确认无问题后逐台“人工确认 OK”；整批全部确认后批次自动更新为迁移完成。`)}</p></div><div className="validation-progress"><span><i style={{ width: `${Math.round((confirmedValidationCount / validationTasks.length) * 100)}%` }} /></span><strong>{t(Math.round((confirmedValidationCount / validationTasks.length) * 100))}%</strong><button onClick={() => setPanel('validation')}>{t("查看今日验证列表 →")}</button></div>
            </div>}

              </div> : !visibleMessages.length ? <section className="stage-chat-intro"><Icon name="chat" size={22} /><h2>{t(`在${active.title}中开启讨论`)}</h2><p>{t('已关联「{0}」的阶段资料与任务。可以直接提问，或从输入框上方选择操作；执行结果会同步到整个项目。', projectName)}</p></section> : null}
            </div>}
            {agentTyping && !isManagement && view.replyStartedAt && <ThinkingIndicator key={conversationId} startedAt={view.replyStartedAt} />}
...
function ValidationPanel({ tasks, completedBatchIds, onConfirm, onConfirmBatch, onClose }: { tasks: ValidationVm[]; completedBatchIds: string[]; onConfirm: (id: string) => void; onConfirmBatch: (ids: string[]) => void; onClose: () => void }) {
  ...
  const visibleTasks = tasks.filter((task) => (statusFilter === '全部' || (statusFilter === '迁移完成' ? task.confirmed : !task.confirmed)) && `${task.batchId}${task.batchPhase}${t(task.batchPhase)}${task.vmName}`.toLowerCase().includes(query.toLowerCase()));
  ...
  if (selectedTask) return <div className="panel-backdrop">
    <aside className="archive-panel validation-panel validation-detail" role="region" aria-label={t(`${selectedTask.vmName} 验证结果详情`)}>
      <header><div><p>VM VALIDATION RESULT</p><h2>{t("虚拟机配置对比结果")}</h2><span>{t(selectedTask.batchId)} · {t(selectedTask.vmName)}</span></div><div className="validation-head-actions"><button onClick={() => setSelectedTask(null)}>{t("← 返回列表")}</button><button onClick={onClose} aria-label={t("关闭")}>×</button></div></header>
      <div className="validation-identity"><div><small>{t("虚拟机名称")}</small><strong>{t(selectedTask.vmName)}</strong></div><div><small>{t("虚拟机 UUID")}</small><strong>{t(selectedTask.uuid)}</strong></div><div><small>{t("操作系统类型")}</small><strong>{t(selectedTask.osType)}</strong></div><div><small>{t("总体对比结果")}</small><strong className="validation-match">✓ {t(selectedTask.comparison)}</strong></div></div>
      <div className="comparison-column-head"><span>{t("配置项")}</span><b>{t("源端配置")}</b><b>{t("目标端配置")}</b><em>{t("结果")}</em></div>
      <div className="comparison-groups">{comparisonGroups.map((group) => <section key={group.title} className="comparison-group"><header><span>{t(group.icon)}</span><h3>{t(group.title)}</h3><em>{t(`${group.rows.length} 项一致`)}</em></header><div>{group.rows.map(([label, source, target]) => <div className="comparison-row" key={label}><span>{t(label)}</span><b>{t(source)}</b><b>{t(target)}</b><em>{t("✓ 一致")}</em></div>)}</div></section>)}</div>
      <div className="validation-approval"><span className={selectedTask.confirmed ? 'approved' : ''}>{t(selectedTask.confirmed ? '✓' : '!')}</span><div><strong>{t(selectedTask.confirmed ? '该虚拟机已人工确认迁移完成' : '请完成最终人工验收')}</strong><p>{t(selectedTask.confirmed ? '确认记录已写入验证时间线，并同步更新批次状态。' : '确认源端与目标端配置对比无问题后，点击“人工确认 OK”。')}</p></div><button disabled={selectedTask.confirmed || selectedTask.comparison !== '一致'} onClick={() => { onConfirm(selectedTask.id); setSelectedTask({ ...selectedTask, confirmed: true }); }}>{t(selectedTask.confirmed ? '已确认 OK' : '人工确认 OK')}</button></div>
    </aside>
  </div>;

  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel validation-panel" role="region" aria-label={t("今日虚拟机验证列表")}>
      <header><div><p>RESULT VALIDATION WORKSPACE</p><h2>{t("今日虚拟机验证列表")}</h2><span>{t("源端与目标端配置自动对比，人工确认后完成迁移")}</span></div><button onClick={onClose} aria-label={t("关闭")}>×</button></header>
      <div className="validation-kpis"><div data-tone="info"><small>{t("今日进入验证")}</small><strong>{t(tasks.length)}</strong><em>{t("已完成割接的虚拟机")}</em></div><div data-tone="success"><small>{t("Agent 对比一致")}</small><strong>{t(tasks.filter((task) => task.comparison === '一致').length)}</strong><em>{t("配置自动验证通过")}</em></div><div data-tone="success"><small>{t("人工已确认")}</small><strong>{t(confirmedCount)}</strong><em>{t(`${tasks.length - confirmedCount} 台待确认`)}</em></div><div data-tone="success"><small>{t("迁移完成批次")}</small><strong>{t(completedBatchIds.length)}</strong><em>{t("全批次验证完成")}</em></div></div>
      <div className="validation-toolbar"><div>{(['全部', '待确认', '迁移完成'] as const).map((status) => <button key={status} className={statusFilter === status ? 'active' : ''} onClick={() => setStatusFilter(status)}>{t(status)} <b>{t(status === '全部' ? tasks.length : status === '迁移完成' ? confirmedCount : tasks.length - confirmedCount)}</b></button>)}<button className="validation-bulk-confirm" disabled={!selectedConfirmableIds.length} onClick={confirmSelected}>{t(`✓ 批量确认 OK${selectedConfirmableIds.length ? `(${selectedConfirmableIds.length})` : ''}`)}</button></div><label><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("查询批次编号、阶段或虚拟机名称")} /></label></div>
      <div className="validation-table-wrap"><table className="validation-table"><thead><tr><th><input type="checkbox" aria-label={t("选择全部待确认虚拟机")} checked={selectableVisibleIds.length > 0 && selectableVisibleIds.every((id) => selectedIds.includes(id))} onChange={(event) => setSelectedIds(event.target.checked ? Array.from(new Set([...selectedIds, ...selectableVisibleIds])) : selectedIds.filter((id) => !selectableVisibleIds.includes(id)))} /></th><th>{t("批次编号")}</th><th>{t("批次阶段")}</th><th>{t("虚拟机名称")}</th><th>{t("源端配置")}</th><th>{t("目标端配置")}</th><th>{t("验证结果对比")}</th><th>{t("验收状态")}</th><th>{t("操作")}</th></tr></thead><tbody>{visibleTasks.length ? visibleTasks.map((task) => <tr key={task.id}><td><input type="checkbox" aria-label={t(`选择 ${task.vmName}`)} disabled={task.confirmed || task.comparison !== '一致'} checked={selectedIds.includes(task.id)} onChange={(event) => setSelectedIds((ids) => event.target.checked ? [...ids, task.id] : ids.filter((id) => id !== task.id))} /></td><td><strong>{t(task.batchId)}</strong></td><td><span className={`batch-phase phase-${task.batchPhase}`}>{t(task.batchPhase)}</span></td><td><strong>{task.vmName}</strong><small>{t(task.osType)}</small></td><td><span className="config-summary">{t(task.source.cpu.totalCores)} · {t(task.source.memory.totalSize)}<small>{t(task.source.disk.size)} · {t(task.source.network.ip)}</small></span></td><td><span className="config-summary">{t(task.target.cpu.totalCores)} · {t(task.target.memory.totalSize)}<small>{t(task.target.disk.size)} · {t(task.target.network.ip)}</small></span></td><td><button className="comparison-link" onClick={() => setSelectedTask(task)}><span>✓ {t(task.comparison)}</span><small>{t("查看结果信息 →")}</small></button></td><td><span className={task.confirmed ? 'validation-status confirmed' : 'validation-status'}>{t(task.confirmed ? '迁移完成' : '待人工确认')}</span></td><td><button className="validation-single-confirm" disabled={task.confirmed || task.comparison !== '一致'} onClick={() => { onConfirm(task.id); setSelectedIds((ids) => ids.filter((id) => id !== task.id)); }}>{t(task.confirmed ? '已确认' : '确认 OK')}</button></td></tr>) : <tr><td colSpan={9} className="empty-row">{t("暂无符合条件的验证任务")}</td></tr>}</tbody></table></div>
      <div className="validation-foot"><span>{t(`共${visibleTasks.length} 条 · 示例配置对比完成`)}</span><p>{t("批次内全部虚拟机人工确认后，批次自动更新为“迁移完成”")}</p></div>
    </aside>
  </div>;
}
...
function CutoverTaskPanel({ tasks, completedIds, running, onComplete, onClose, onNotify }: { tasks: VmTask[]; completedIds: string[]; running: boolean; onComplete: (taskIds: string[]) => void; onClose: () => void; onNotify: (message: string) => void }) {
  ...
  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel task-panel cutover-panel" role="region" aria-label={t("今日待割接任务")}>
      <header><div><p>TODAY&apos;S CUTOVER QUEUE</p><h2>{t("今日待割接任务")}</h2><span>{t("任务详情与任务管理中的单虚拟机执行记录保持一致")}</span></div><button onClick={onClose} aria-label={t("关闭")}>×</button></header>
      <div className="cutover-kpis"><div data-tone="info"><small>{t("待割接任务")}</small><strong>{t(tasks.length)}</strong><em>{t("来自今日割接验证批次")}</em></div><div data-tone="success"><small>{t("同步已完成")}</small><strong>{t(tasks.filter((task) => task.status === '成功').length)}</strong><em>{t("可进入割接前检查")}</em></div><div data-tone="brand"><small>{t("同步处理中")}</small><strong>{t(tasks.filter((task) => task.status === '同步中').length)}</strong><em>{t("持续监控数据状态")}</em></div><div data-tone="brand"><small>{t("已选择")}</small><strong>{t(selectedIds.length)}</strong><em>{t("支持批量发起割接")}</em></div></div>
      <div className="vm-task-tabs">{['全部', '成功', '待同步', '同步中', '暂停'].map((status) => <button key={status} className={statusFilter === status ? 'active' : ''} onClick={() => setStatusFilter(status)}>{t(status)} <b>{t(statusCount(status))}</b></button>)}</div>
      <p className="shared-task-note" role="status">{t(running ? "割接队列正在执行，所有会话共享此状态。" : `已完成 ${completedIds.length} / ${tasks.length} 个割接任务。`)}</p><div className="vm-task-toolbar"><div><button disabled={running || !actionableIds.length} onClick={() => onNotify(`已对 ${selectedIds.length} 个任务发起割接前检查`)}>{t("✓ 割接前检查")}</button><button className="complete-cutover" disabled={running || !actionableIds.length} onClick={() => { onComplete(actionableIds); setSelectedIds([]); }}>{t("✓ 确认割接完成")}</button><button onClick={() => onNotify('待割接任务报告已导出（演示）')}>{t("⇩ 导出报告")}</button></div><label><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("输入任务名称、目标 IP 或任务编号")} /></label><button onClick={() => { setQuery(''); setStatusFilter('全部'); }}>↻</button></div>
      <div className="vm-table-wrap"><table className="vm-task-table"><thead><tr><th><input type="checkbox" disabled={running || !selectableIds.length} checked={selectableIds.length > 0 && selectableIds.every((id) => selectedIds.includes(id))} onChange={(event) => setSelectedIds(event.target.checked ? selectableIds : [])} aria-label={t("选择全部待割接任务")} /></th><th>{t("任务名称")}</th><th>{t("迁移目标 IP")}</th><th>{t("任务状态")}</th><th>{t("校验状态")}</th><th>{t("任务进度")}</th><th>{t("当前已迁移/总量")}</th><th>{t("迁移速率")}</th><th>{t("开始时间")}</th><th>{t("结束时间")}</th><th>{t("耗时")}</th><th>{t("剩余迁移时间")}</th><th>{t("操作")}</th></tr></thead><tbody>{visibleTasks.length ? visibleTasks.map((task) => <tr key={task.id}><td><input type="checkbox" disabled={running || completedIds.includes(task.id)} checked={selectedIds.includes(task.id)} onChange={(event) => setSelectedIds((ids) => event.target.checked ? [...ids, task.id] : ids.filter((id) => id !== task.id))} aria-label={t(`选择 ${task.name}`)} /></td><td><strong>{task.name}</strong><small>{t(task.id)}</small></td><td>{t(task.targetIp)}</td><td><span className={`vm-status status-${task.status}`}>{t(completedIds.includes(task.id) ? '割接完成' : running ? '割接执行中' : task.status)}</span></td><td>{t(task.checkStatus)}</td><td><div className="vm-progress"><span><i style={{ width: `${task.progress}%` }} /></span><b>{t(task.progress)}%</b></div></td><td>{t(task.migrated)}</td><td>{t(task.speed)}</td><td>{t(task.startTime)}</td><td>{t(task.endTime)}</td><td>{t(task.duration)}</td><td>{t(task.remaining)}</td><td><button className="vm-more" onClick={() => onNotify(`${task.name} 割接任务详情已打开`)}>{t("详情 ···")}</button></td></tr>) : <tr><td className="empty-row" colSpan={13}>{t("暂无符合条件的待割接任务")}</td></tr>}</tbody></table></div>
      <div className="vm-table-foot"><span>{t(`总条数：${visibleTasks.length} · 今日割接窗口 22:00–次日 02:00`)}</span><div><button>{t("10 / 页⌄")}</button><button>‹</button><b>1</b><button>›</button></div></div>
    </aside>
  </div>;
}
...
function SyncTaskPanel({ batches, total, completed, onClose }: { batches: BatchTask[]; total: number; completed: number; onClose: () => void }) {
  ...
  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel task-panel sync-panel" role="region" aria-label={t("今日待增量同步任务")}>
      <header><div><p>TODAY&apos;S INCREMENTAL SYNC</p><h2>{t("今日待增量同步任务")}</h2><span>{t("查看每台虚拟机的同步窗口、源端快照与目标端信息")}</span></div><button onClick={onClose} aria-label={t("关闭")}>×</button></header>
      <div className="cutover-kpis"><div data-tone="info"><small>{t("同步任务")}</small><strong>{t(rows.length)}</strong><em>{t("来自迁移批次计划")}</em></div><div data-tone="success"><small>{t("已完成")}</small><strong>{t(Math.min(completed, rows.length))}</strong><em>{t("数据校验通过")}</em></div><div data-tone="brand"><small>{t("运行中")}</small><strong>{t(completed < rows.length && completed > 0 ? 1 : 0)}</strong><em>{t("持续同步变化块")}</em></div><div data-tone="info"><small>{t("待执行")}</small><strong>{t(Math.max(0, rows.length - completed))}</strong><em>{t("等待同步窗口")}</em></div></div>
      <div className="sync-toolbar"><p>{t("增量同步任务完成后，任务状态仍可在此查看。")}</p><label><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("查询任务编号、批次或虚拟机")} /></label></div>
      <div className="sync-table-wrap"><table className="sync-table"><thead><tr><th>{t("任务编号")}</th><th>{t("批次编号")}</th><th>{t("批次阶段")}</th><th>{t("虚拟机名称")}</th><th>{t("源端快照")}</th><th>{t("目标端 IP")}</th><th>{t("同步数据量")}</th><th>{t("计划窗口")}</th><th>{t("任务状态")}</th></tr></thead><tbody>{visible.length ? visible.map((row, index) => { const done = index < completed; const running = !done && index === completed && completed > 0; return <tr key={row.id}><td><strong>{t(row.id)}</strong></td><td>{t(row.batchId)}</td><td><span className={`batch-phase phase-${row.batchPhase}`}>{t(row.batchPhase)}</span></td><td>{row.vmName}</td><td>{t(row.source)}</td><td>{t(row.target)}</td><td>{t(row.size)}</td><td>{t(row.window)}</td><td><span className={`sync-status ${done ? 'done' : running ? 'running' : ''}`}>{t(done ? '已完成' : running ? '运行中' : '待执行')}</span></td></tr>; }) : <tr><td colSpan={9} className="empty-row">{t("暂无符合条件的增量同步任务")}</td></tr>}</tbody></table></div>
    </aside>
  </div>;
}
...
function CreationTaskPanel({ tasks, locked, onChange, onClose, onNotify }: { tasks: CreationTask[]; locked: boolean; onChange: (tasks: CreationTask[]) => void; onClose: () => void; onNotify: (message: string) => void }) {
  ...
  if (editingTask && draftTask) return <div className="panel-backdrop">
    <aside className="archive-panel creation-panel creation-wizard" role="region" aria-label={t("创建迁移任务")}>
      <header><div><p>CREATE MIGRATION TASK</p><h2>{t("配置迁移任务")}</h2><span>{t(editingTask.vmName)} · {t(editingTask.id)}</span></div><button onClick={() => setEditingTask(null)} aria-label={t("关闭配置")}>×</button></header>
      <div className="wizard-steps"><button className={step === 1 ? 'active' : step > 1 ? 'done' : ''} onClick={() => setStep(1)}><i>{t(step > 1 ? '✓' : '1')}</i><span>{t("任务配置")}</span></button><b /><button className={step === 2 ? 'active' : step > 2 ? 'done' : ''} onClick={() => step > 1 && setStep(2)}><i>{t(step > 2 ? '✓' : '2')}</i><span>{t("目的 VM 与网络配置")}</span></button><b /><button className={step === 3 ? 'active' : ''} onClick={() => step > 2 && setStep(3)}><i>3</i><span>{t("确认信息")}</span></button></div>
      <div className="wizard-body">
        {step === 1 && <div className="wizard-section"><div className="wizard-section-head"><small>STEP 1</small><h3>{t("确认源端虚拟机与任务信息")}</h3><p>{t("源端检查已通过，可修改任务名称后进入目的端配置。")}</p></div><div className="source-summary"><div><small>{t("源端主机")}</small><strong>{draftTask.hostName}</strong></div><div><small>{t("虚拟机")}</small><strong>{draftTask.vmName}</strong></div><div><small>{t("操作系统")}</small><strong>{t(draftTask.os)}</strong></div><div><small>{t("检查结果")}</small><strong className="check-pass">{t("✓ 通过")}</strong></div></div><div className="wizard-form"><label className="wide"><span>{t("任务名称")}</span><input value={draftTask.taskName} onChange={(event) => setDraftTask({ ...draftTask, taskName: event.target.value })} /></label><label><span>{t("固件版本")}</span><Select aria-label={t("固件版本")} value={draftTask.firmware} onValueChange={(value) => setDraftTask({ ...draftTask, firmware: value as CreationTask['firmware'] })}><option value={"BIOS"}>BIOS</option><option value={"UEFI"}>UEFI</option></Select></label><label><span>{t("迁移策略")}</span><Select aria-label={t("迁移策略")} defaultValue="按计划执行"><option value={"按计划执行"}>{t("按计划执行")}</option><option value={"立即执行"}>{t("立即执行")}</option></Select></label></div></div>}
        {step === 2 && <div className="wizard-section"><div className="wizard-section-head"><small>STEP 2</small><h3>{t("目的 VM 与网络配置")}</h3><p>{t("确认目标计算资源、规格、磁盘和端口组映射。")}</p></div><div className="resource-hints"><span>{t("推荐规格")}</span><b>{t(`CPU：${draftTask.cpu} 核`)}</b><b>{t(`内存：${draftTask.memory}`)}</b></div><div className="wizard-form"><label className="wide"><span>{t("目的虚拟机名称")}</span><input value={draftTask.vmName} onChange={(event) => setDraftTask({ ...draftTask, vmName: event.target.value })} /></label><label className="wide"><span>{t("计算资源")}</span><Select aria-label={t("计算资源")} value={computeResource} onValueChange={setComputeResource}><option value={"FusionCompute-生产集群-A"}>{t("FusionCompute-生产集群-A")}</option><option value={"FusionCompute-生产集群-B"}>{t("FusionCompute-生产集群-B")}</option></Select></label><label className="wide"><span>{t("操作系统版本")}</span><input value={draftTask.os} onChange={(event) => setDraftTask({ ...draftTask, os: event.target.value })} /></label><label><span>{t("CPU 核数")}</span><input type="number" min="1" value={draftTask.cpu} onChange={(event) => setDraftTask({ ...draftTask, cpu: Number(event.target.value) })} /></label><label><span>{t("内存大小")}</span><input value={draftTask.memory} onChange={(event) => setDraftTask({ ...draftTask, memory: event.target.value })} /></label><label><span>{t("显卡")}</span><Select aria-label={t("显卡")} defaultValue="VGA · 4MB"><option value={"VGA · 4MB"}>VGA · 4MB</option><option value={"VGA · 8MB"}>VGA · 8MB</option></Select></label><label className="wide"><span>{t("网卡迁移 / 端口组")}</span><Select aria-label={t("网卡迁移 / 端口组")} value={portGroup} onValueChange={setPortGroup}><option value={"managePortgroup"}>managePortgroup</option><option value={"business-prod-vlan"}>business-prod-vlan</option><option value={"database-backend-vlan"}>database-backend-vlan</option></Select></label><label className="wide"><span>{t("磁盘配置")}</span><input value={draftTask.disk} onChange={(event) => setDraftTask({ ...draftTask, disk: event.target.value })} /></label></div><div className="advanced-options"><label><button type="button" className={delayAllocation ? 'on' : ''} onClick={() => setDelayAllocation((value) => !value)}><i /></button><span><strong>{t("延迟分配")}</strong><small>{t("按需分配目标存储空间")}</small></span></label><label><button type="button" className={keepUuid ? 'on' : ''} onClick={() => setKeepUuid((value) => !value)}><i /></button><span><strong>{t("UUID 与源端保持一致")}</strong><small>{t("保留源端虚拟机唯一标识")}</small></span></label></div></div>}
        {step === 3 && <div className="wizard-section confirm-section"><div className="wizard-section-head"><small>STEP 3</small><h3>{t("确认迁移任务信息")}</h3><p>{t("确认无误后，该任务将进入批量任务创建队列。")}</p></div><div className="confirm-grid"><div><small>{t("任务名称")}</small><strong>{draftTask.taskName}</strong></div><div><small>{t("源端虚拟机")}</small><strong>{t(editingTask.vmName)}</strong></div><div><small>{t("目的虚拟机")}</small><strong>{draftTask.vmName}</strong></div><div><small>{t("计算资源")}</small><strong>{computeResource}</strong></div><div><small>{t("CPU / 内存")}</small><strong>{t(`${draftTask.cpu} 核 /${draftTask.memory}`)}</strong></div><div><small>{t("磁盘")}</small><strong>{t(draftTask.disk)}</strong></div><div><small>{t("网络端口组")}</small><strong>{t(portGroup)}</strong></div><div><small>{t("高级配置")}</small><strong>{t(delayAllocation ? '延迟分配' : '预分配')} · {t(keepUuid ? '保留 UUID' : '新建 UUID')}</strong></div></div><div className="confirm-notice"><span>!</span><p><strong>{t("确认后仍可在批量创建前修改")}</strong>{t("所有任务均确认后，“批量创建任务”按钮才会开放。")}</p></div></div>}
      </div>
      <div className="wizard-foot"><button onClick={() => step === 1 ? setEditingTask(null) : setStep((value) => value - 1)}>{t(step === 1 ? '取消' : '上一步')}</button>{step < 3 ? <button className="primary" onClick={() => setStep((value) => value + 1)} disabled={step === 1 && !draftTask.taskName.trim()}>{t("下一步 →")}</button> : <button className="primary" onClick={confirmTask}>{t("确认任务信息")}</button>}</div>
    </aside>
  </div>;

  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel creation-panel" role="region" aria-label={t("待新建迁移任务")}>
      <header><div><p>NEW MIGRATION TASK QUEUE</p><h2>{t("今日待新建任务")}</h2><span>{t("逐台查看并确认目的 VM 配置，全部确认后批量创建")}</span></div><button onClick={onClose} aria-label={t("关闭")}>×</button></header>
      <div className="creation-summary"><div data-tone="info"><small>{t("待新建")}</small><strong>{t(tasks.filter((task) => task.status === '待配置').length)}</strong></div><div data-tone="brand"><small>{t("已确认")}</small><strong>{t(tasks.filter((task) => task.status === '已确认').length)}</strong></div><div data-tone="success"><small>{t("已创建")}</small><strong>{t(tasks.filter((task) => task.status === '已创建').length)}</strong></div><span><i />{t("源端检查通过，任务信息来自今日迁移批次")}</span></div>
      <p className="shared-task-note" role="status">{t(locked ? "任务已提交，所有会话共享执行状态，配置已锁定。" : "确认的配置会同步到当前项目的所有会话。")}</p><div className="creation-toolbar"><div><button className="bulk-confirm" disabled={locked || !selectedIds.length || !tasks.some((task) => selectedIds.includes(task.id) && task.status === '待配置')} onClick={confirmSelectedTasks}>{t("✓ 批量确认")}</button><button className="primary" disabled={locked || !allConfirmed || tasks.every((task) => task.status === '已创建')} onClick={createAllTasks}>{t("＋ 批量创建任务")}</button><span>{t(`已选择${selectedIds.length}`)}</span></div><label><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("输入虚拟机或任务名称")} /></label></div>
      <div className="creation-table-wrap"><table className="creation-table"><thead><tr><th><input type="checkbox" checked={selectedIds.length === tasks.length && tasks.length > 0} onChange={(event) => setSelectedIds(event.target.checked ? tasks.map((task) => task.id) : [])} aria-label={t("选择全部")} /></th><th>{t("主机名称")}</th><th>{t("虚拟机名称")}</th><th>{t("状态")}</th><th>{t("操作系统")}</th><th>{t("固件版本")}</th><th>CPU</th><th>{t("内存")}</th><th>{t("磁盘")}</th><th>{t("VMTools 状态")}</th><th>{t("检查结果")}</th><th>{t("任务名称")}</th><th>{t("任务状态")}</th><th>{t("操作")}</th></tr></thead><tbody>{visibleTasks.map((task) => <tr key={task.id}><td><input type="checkbox" checked={selectedIds.includes(task.id)} onChange={(event) => setSelectedIds((ids) => event.target.checked ? [...ids, task.id] : ids.filter((id) => id !== task.id))} aria-label={t(`选择 ${task.vmName}`)} /></td><td>{task.hostName}</td><td><strong>{task.vmName}</strong><small>{t(task.id)}</small></td><td>{t(task.powerState)}</td><td>{t(task.os)}</td><td>{t(task.firmware)}</td><td>{t(task.cpu)}</td><td>{t(task.memory)}</td><td>{t(task.disk)}</td><td>{t(task.vmtools)}</td><td><span className="check-pass">✓ {t(task.check)}</span></td><td>{task.taskName}</td><td><span className={`creation-status status-${task.status}`}>{t(task.status)}</span></td><td><button className="configure-task" disabled={locked || task.status === '已创建'} onClick={() => openWizard(task)}>{t(task.status === '待配置' ? '配置任务' : task.status === '已确认' ? '查看 / 修改' : '已创建')}</button></td></tr>)}</tbody></table></div>
      <div className="creation-foot"><span>{t(`共${visibleTasks.length} 条 · 已确认${confirmedCount}/${tasks.length}`)}</span><p>{t(allConfirmed ? '全部任务已确认，可以批量创建' : `还有 ${tasks.length - confirmedCount} 个任务需要确认`)}</p></div>
    </aside>
  </div>;
}
...
function TaskPanel({ projectId, batches, creationTasks, risks, completedBatchIds, projectExists, onClose, onNotify }: { projectId: string; batches: BatchTask[]; creationTasks: CreationTask[]; risks: RiskItem[]; completedBatchIds: string[]; projectExists: boolean; onClose: () => void; onNotify: (message: string) => void }) {
  ...
  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel task-panel" role="region" aria-label={t("任务管理")}>
      <header><div><p>MIGRATION TASK MANAGEMENT</p><h2>{t("迁移任务管理")}</h2><span>{t(activeTab === 'batches' ? '按批次管理迁移计划，并下钻查看虚拟机任务' : activeBatch ? `${activeBatch.id} · ${activeBatch.batchPhase} · ${activeBatch.stageType}` : '等待规划子智能体生成迁移批次')}</span></div><div className="task-head-actions"><button onClick={onClose} aria-label={t("关闭")}>×</button></div></header>
      <nav className="task-primary-tabs" aria-label={t("迁移任务管理视图")}><button className={activeTab === 'batches' ? 'active' : ''} onClick={() => { setActiveTab('batches'); setSelectedVm(null); }}>{t("迁移批次")}<b>{t(batches.length)}</b></button><button className={activeTab === 'tasks' ? 'active' : ''} onClick={() => openTaskTab()}>{t("迁移任务")}<b>{t(totalVms)}</b></button></nav>

      {activeTab === 'batches' ? <>
        <div className="panel-kpis"><div data-tone="brand"><small>{t("批次任务")}</small><strong>{t(batches.length)}</strong><em>{t(projectExists ? '规划子智能体生成' : '等待规划设计')}</em></div><div data-tone="info"><small>{t("虚拟机范围")}</small><strong>{t(totalVms)}</strong><em>{t("覆盖全部规划范围")}</em></div><div data-tone="info"><small>{t("批次阶段")}</small><strong>{t(batches.length ? 3 : 0)}</strong><em>{t("试点 · 攻坚 · 扩展")}</em></div></div>
        <div className="batch-toolbar"><div>{(['全部', '试点', '攻坚', '扩展'] as const).map((phase) => <button key={phase} className={batchPhaseFilter === phase ? 'active' : ''} onClick={() => { setBatchPhaseFilter(phase); setSelectedGanttBatchId(null); }}>{t(phase === '全部' ? '全部批次' : phase)} {t(phase === '全部' ? batches.length : batches.filter((batch) => batch.batchPhase === phase).length)}</button>)}</div><button className="gantt-button" disabled={!batches.length} onClick={() => { setShowGantt((value) => !value); if (!showGantt) onNotify('迁移批次甘特图已生成'); }}>{t(showGantt ? '收起甘特图' : '▤ 生成甘特图')}</button><button onClick={() => onNotify('批次任务由规划子智能体自动生成')}>{t("＋ 创建任务")}</button></div>
        {showGantt && <div className="gantt-card"><div className="gantt-head"><div><small>MIGRATION BATCH TIMELINE</small><h3>{t(batchPhaseFilter === '全部' ? '全部迁移批次甘特图' : `${batchPhaseFilter}阶段批次甘特图`)}</h3></div><span>{t(`2026 年 9 月 · 共${visibleBatches.length} 个批次`)}</span></div><div className="gantt-scroll"><div className="gantt-calendar"><div className="gantt-date-row"><span>{t("批次 / 阶段类型")}</span><div>{ganttDays.map((day) => <b key={day}>{t(`${day}日`)}</b>)}</div></div>{visibleBatches.map((batch) => { const startDay = Number(batch.startDate.slice(-2)); const left = ((startDay - 7) / ganttDays.length) * 100; const width = (batch.durationDays / ganttDays.length) * 100; return <div className={`gantt-row ${selectedGanttBatchId === batch.id ? 'selected' : ''}`} key={batch.id}><span><strong>{t(batch.id)}</strong><small>{t(batch.batchPhase)} · {t(batch.stageType)}</small></span><div className="gantt-track">{ganttDays.map((day) => <i key={day} />)}<button className={`gantt-bar phase-${batch.batchPhase} ${selectedGanttBatchId === batch.id ? 'selected' : ''}`} style={{ left: `${left}%`, width: `${width}%` }} aria-pressed={selectedGanttBatchId === batch.id} onClick={() => { setSelectedGanttBatchId(batch.id); window.setTimeout(() => document.getElementById(`batch-row-${projectId}-${batch.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 0); }}><b>{t(`${batch.durationDays}天 ·${batch.vmNames.length}台`)}</b></button></div></div>; })}</div></div><div className="gantt-legend"><span><i className="pilot" />{t("试点")}</span><span><i className="tackle" />{t("攻坚")}</span><span><i className="expand" />{t("扩展")}</span></div></div>}
        <div className="batch-table-wrap"><table className="batch-table"><thead><tr><th>{t("批次编号")}</th><th>{t("批次阶段")}</th><th>{t("虚拟机数量")}</th><th>{t("虚拟机列表")}</th><th>{t("阶段类型")}</th><th>{t("开始日期")}</th><th>{t("结束日期")}</th><th>{t("持续天数")}</th><th>{t("状态")}</th></tr></thead><tbody>{visibleBatches.length ? visibleBatches.map((batch) => <tr id={`batch-row-${projectId}-${batch.id}`} className={selectedGanttBatchId === batch.id ? 'selected-batch' : ''} key={batch.id}><td><strong>{t(batch.id)}</strong></td><td><span className={`batch-phase phase-${batch.batchPhase}`}>{t(batch.batchPhase)}</span></td><td><strong className="vm-count">{t(`${batch.vmNames.length} 台`)}</strong></td><td><button className="vm-list-link" onClick={() => openTaskTab(batch)}><b>{t("清单")}</b><small>{batch.vmNames.slice(0, 2).join('、')}{batch.vmNames.length > 2 ? '…' : ''}</small><em>{t("查看明细 →")}</em></button></td><td>{t(batch.stageType)}</td><td>{t(batch.startDate)}</td><td>{t(batch.endDate)}</td><td>{t(`${batch.durationDays} 天`)}</td><td><span className={completedBatchIds.includes(batch.id) ? 'task-ready batch-complete' : 'task-ready'}><i />{t(completedBatchIds.includes(batch.id) ? '迁移完成' : '已规划')}</span></td></tr>) : <tr><td className="empty-row" colSpan={9}>{t(batches.length ? `暂无${batchPhaseFilter}阶段的批次任务` : projectExists ? '完成规划信息上传后，批次任务将在这里自动生成' : '请先创建项目')}</td></tr>}</tbody></table></div>
      </> : selectedVm ? <div className="vm-detail-view">
        <div className="vm-detail-head"><button onClick={() => setSelectedVm(null)}>{t("← 返回迁移任务")}</button><div><small>VM MIGRATION DETAIL</small><h3>{t(selectedVm.name)}</h3><p>{t(selectedVm.id)} · {t(activeBatch?.id)} · {t(isCreated(selectedVm.name) ? '任务已创建' : '任务尚未创建')}</p></div><span className={isCreated(selectedVm.name) ? 'created' : 'not-created'}>{t(isCreated(selectedVm.name) ? '已创建迁移任务' : '待创建迁移任务')}</span></div>
        {isCreated(selectedVm.name) ? <><div className="vm-detail-summary"><div><small>{t("虚拟机 ID")}</small><strong>{t(selectedVm.id)}</strong></div><div><small>{t("虚拟机名称")}</small><strong>{t(selectedVm.name)}</strong></div><div><small>{t("操作系统版本")}</small><strong>{t(selectedVmOs)}</strong></div><div><small>{t("迁移方案")}</small><strong>{t("免代理")}</strong></div></div><div className="created-task-list"><div className="created-task-list-head"><div><small>MIGRATION TASK LIST</small><h3>{t("对应迁移任务列表")}</h3></div><span>{t(`共${migrationTaskRows.length} 项`)}</span></div><table><thead><tr><th>{t("任务编号")}</th><th>{t("任务类型")}</th><th>{t("任务状态")}</th><th>{t("任务进度")}</th><th>{t("开始时间")}</th><th>{t("结束时间")}</th></tr></thead><tbody>{migrationTaskRows.map((row) => <tr key={row[0]}>{row.map((cell, index) => <td key={`${row[0]}-${index}`}>{index === 2 ? <span className={`detail-task-status ${cell === '已完成' ? 'done' : cell === '运行中' ? 'running' : ''}`}>{t(cell)}</span> : cell}</td>)}</tr>)}</tbody></table></div></> : <div className="uncreated-vm-detail"><div className="vm-detail-summary"><div><small>{t("虚拟机 ID")}</small><strong>{t(selectedVm.id)}</strong></div><div><small>{t("虚拟机名称")}</small><strong>{t(selectedVm.name)}</strong></div><div><small>{t("操作系统版本")}</small><strong>{t(selectedVmOs)}</strong></div><div><small>{t("迁移方案")}</small><strong>{t("免代理")}</strong></div></div><section><header><div><small>RISK ITEMS</small><h3>{t("风险项")}</h3></div><span>{t(`${selectedVmRisks.length} 项待处理`)}</span></header>{selectedVmRisks.length ? <div className="vm-risk-list">{selectedVmRisks.map((risk) => <div key={risk.id}><b className={`risk-level level-${risk.level}`}>{t(risk.level)}</b><p><strong>{t(risk.description)}</strong><small>R-{t(String(risk.id).padStart(3, '0'))} · {t(stageName[risk.stage])} · {t(risk.closed ? '已闭环' : '待闭环')}</small></p></div>)}</div> : <div className="vm-no-risk"><span>✓</span><p><strong>{t("当前虚拟机未关联未闭环风险")}</strong><small>{t("任务创建前仍将执行兼容性与资源检查。")}</small></p></div>}</section></div>}
      </div> : <>
        <div className="migration-task-context"><div><small>{t("当前批次")}</small><Select aria-label={t('当前批次')} value={activeBatch?.id ?? ''} onValueChange={(value) => { setSelectedBatch(batches.find((batch) => batch.id === value) ?? null); setQuery(''); setStatusFilter('全部'); setSelectedVmIds([]); }} disabled={!batches.length}>{batches.length ? batches.map((batch) => <option key={batch.id} value={batch.id}>{t(batch.id)} · {t(batch.batchPhase)} · {t(batch.stageType)}</option>) : <option value={"暂无迁移批次"}>{t("暂无迁移批次")}</option>}</Select></div><p><span><i className="created" />{t(`已创建${createdCount}`)}</span><span><i />{t(`未创建${Math.max(0, vmTasks.length - createdCount)}`)}</span></p></div>
        <div className="vm-task-tabs">{(['全部', '已创建', '未创建'] as const).map((status) => { const count = status === '全部' ? vmTasks.length : status === '已创建' ? createdCount : vmTasks.length - createdCount; return <button key={status} className={statusFilter === status ? 'active' : ''} onClick={() => setStatusFilter(status)}>{t(status)} <b>{t(count)}</b></button>; })}</div>
        <div className="vm-task-toolbar migration-task-toolbar"><div className="migration-bulk-actions">{['同步', '暂停', '删除', '定时同步', '日志下载', '取消定时同步'].map((action) => <button key={action} disabled={!selectedVmIds.length} onClick={() => runMigrationTaskAction(action)}>{t(action)}</button>)}<span>{t(`已选择${selectedVmIds.length}`)}</span></div><label><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("输入虚拟机 ID、名称或目标 IP")} /></label><button onClick={() => { setQuery(''); setStatusFilter('全部'); }}>↻</button></div>
        <div className="migration-vm-table-wrap"><table className="migration-vm-table"><thead><tr><th><input type="checkbox" aria-label={t("选择全部迁移任务")} checked={visibleVmTasks.length > 0 && visibleVmTasks.every((task) => selectedVmIds.includes(task.id))} onChange={(event) => setSelectedVmIds(event.target.checked ? Array.from(new Set([...selectedVmIds, ...visibleVmTasks.map((task) => task.id)])) : selectedVmIds.filter((id) => !visibleVmTasks.some((task) => task.id === id)))} /></th><th>{t("虚拟机 ID")}</th><th>{t("虚拟机名称")}</th><th>{t("操作系统版本")}</th><th>{t("迁移方案")}</th><th>{t("任务创建状态")}</th><th>{t("风险项")}</th><th>{t("目标 IP")}</th><th>{t("功能操作")}</th></tr></thead><tbody>{visibleVmTasks.length ? visibleVmTasks.map((task, index) => { const creation = creationByVm.get(task.name); const riskCount = risks.filter((risk) => !risk.closed && (risk.stage === 'research' || risk.stage === 'planning')).slice(index % 2, index % 2 + 2).length; const os = creation?.os ?? (index % 3 === 0 ? 'Debian GNU/Linux 11' : index % 3 === 1 ? 'CentOS 7 (64 位)' : 'Windows Server 2019'); return <tr key={task.id}><td><input type="checkbox" aria-label={t(`选择 ${task.name}`)} checked={selectedVmIds.includes(task.id)} onChange={(event) => setSelectedVmIds((ids) => event.target.checked ? [...ids, task.id] : ids.filter((id) => id !== task.id))} /></td><td><strong>{t(task.id)}</strong></td><td>{task.name}</td><td>{t(os)}</td><td><span className="agentless-tag">{t("免代理")}</span></td><td><span className={`creation-state ${isCreated(task.name) ? 'created' : ''}`}><i />{t(isCreated(task.name) ? '已创建' : '未创建')}</span></td><td>{riskCount ? <span className="vm-risk-count">{t(`${riskCount} 项`)}</span> : '—'}</td><td>{t(task.targetIp)}</td><td><div className="migration-row-actions"><button className="detail" onClick={() => setSelectedVm(task)}>{t("查看明细")}</button>{['同步', '暂停', '删除', '定时同步', '日志下载', '取消定时同步'].map((action) => <button key={action} onClick={() => runMigrationTaskAction(action, task.id)}>{t(action)}</button>)}</div></td></tr>; }) : <tr><td className="empty-row" colSpan={9}>{t(batches.length ? '暂无符合条件的虚拟机' : '完成规划设计后显示迁移任务')}</td></tr>}</tbody></table></div>
        <div className="vm-table-foot"><span>{t(`总条数：${visibleVmTasks.length}`)}</span><div><button>{t("10 / 页⌄")}</button><button>‹</button><b>1</b><button>›</button></div></div>
      </>}
    </aside>
  </div>;
}
...
function RiskPanel({ projectId, risks, projectExists, onCloseRisk }: { projectId: string; risks: RiskItem[]; projectExists: boolean; onClose: () => void; onCloseRisk: (id: number, closureDescription: string) => void }) {
  ...
  const visible = risks.filter((risk) => (level === '全部' || risk.level === level) && `${risk.description}${t(risk.description)}${stageName[risk.stage]}${t(stageName[risk.stage])}${risk.batchId}${risk.vmName}${risk.vmId}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <section className="archive-panel risk-panel" aria-label={t("交付风险清单")}>
    <header><div><h2>{t("交付风险")}</h2><span>{t(highOpen ? `${highOpen} 项高风险需要处理，闭环后可推进到下一阶段。` : risks.length ? '高风险已闭环，可以继续推进。' : '完成评估后，在这里查看风险与处理建议。')}</span></div></header>
    {risks.length > 0 && <section className="risk-overview" aria-label={t("项目风险闭环概况")}>
      <header><h3>{t("项目风险闭环")}</h3><span>{t(`${closedCount} /${risks.length} 项已完成`)}</span></header>
      ...
      <div className="risk-chart-legend"><span><i />{t(`已闭环${closedCount}`)}</span><span><i />{t(`待闭环${risks.length - closedCount}`)}</span><span>{t(`高风险待处理${highOpen}`)}</span></div>
    </section>}
    <div className="risk-toolbar"><label><Icon name="search" size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("搜索风险、批次或虚拟机")} aria-label={t("搜索交付风险")} /></label><div>{['全部', '高', '中', '低'].map((item) => <button key={item} className={level === item ? 'active' : ''} aria-pressed={level === item} onClick={() => setLevel(item)}>{t(item === '全部' ? '全部' : `${item}风险`)}</button>)}</div></div>
    <div className="risk-list">{visible.length ? visible.map((risk) => <article className={`risk-item ${risk.closed ? 'is-closed' : ''}`} key={risk.id}>
      <div className="risk-item-heading"><span className={`risk-severity level-${risk.level}`}>{t(risk.level)}</span><h3>{t(risk.description)}</h3><button disabled={risk.closed} onClick={() => { setClosingRiskId(risk.id); setClosureDescription(''); }}>{t(risk.closed ? '已闭环' : '处理风险')}</button></div>
      <div className="risk-item-meta"><span>R-{t(String(risk.id).padStart(3, '0'))}</span><span>{t(stageName[risk.stage])}</span><span>{t(risk.batchId)}</span><span>{risk.closed ? <><Icon name="check" size={12} /> {t(" 已闭环")}</> : '待闭环'}</span></div>
      <details className="risk-item-details"><summary>{t("查看影响范围与记录")}<Icon name="chevron" size={12} /></summary><dl><div><dt>{t("虚拟机")}</dt><dd>{t(risk.vmName)} · {t(risk.vmId)}</dd></div><div><dt>{t("闭环说明")}</dt><dd>{t(risk.closed ? risk.closureDescription : '尚未填写')}</dd></div>{risk.closed && <div><dt>{t("闭环时间")}</dt><dd>{t(risk.closedAt)}</dd></div>}</dl></details>
      {closingRiskId === risk.id && <form className="inline-risk-form" onSubmit={confirmClosure}><label htmlFor={`closure-${projectId}-${risk.id}`}>{t("处理措施与验证结果")}</label><textarea id={`closure-${projectId}-${risk.id}`} value={closureDescription} onChange={(event) => setClosureDescription(event.target.value)} placeholder={t("说明采取的措施、验证结果或闭环依据…")} maxLength={300} autoFocus required /><div><span>{closureDescription.length} / 300</span><button type="button" onClick={() => setClosingRiskId(null)}>{t("取消")}</button><button type="submit" className="primary" disabled={!closureDescription.trim()}>{t("确认闭环")}</button></div></form>}
    </article>) : <p className="inline-empty">{t(projectExists ? '没有符合条件的风险。' : '先创建项目并完成评估。')}</p>}</div>
  </section>;
}
