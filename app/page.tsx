...
function ProjectWorkspace({ project, projects, projectId, onSelectProject, onNewProject }: { project: ProjectInfo | null; projects: ProjectEntry[]; projectId: string; onSelectProject: (id: string) => void; onNewProject: () => void }) {
  ...
  const stageStats = ({
    research: [{ label: '迁移范围', value: assessed ? `${vmCount} 台虚拟机` : '待评估' }, { label: '输入资料', value: `${Number(Boolean(files.rvtools)) + Number(Boolean(files.presales))} / 2 份` }, { label: '识别风险', value: assessed ? `${risks.filter((r) => r.stage === 'research').length} 项` : '待评估' }, { label: '待闭环高风险', value: assessed ? `${researchHigh} 项` : '待评估' }],
    planning: [{ label: '迁移范围', value: planningStatus === 'scope-review' ? `${vmCount} 台 · 待确认` : `${vmCount} 台 · 已确认` }, { label: '迁移批次', value: `${batchTasks.length} 批` }, { label: '规划高风险', value: `${planningHigh} 项待闭环` }, { label: '批次确认', value: batchConfirmation === 'confirmed' ? '已确认' : '待确认' }],
    migration: [{ label: '新建任务', value: `${executionMetrics.creation.completed} / ${executionMetrics.creation.total}` }, { label: '增量同步', value: `${executionMetrics.sync.completed} / ${executionMetrics.sync.total}` }, { label: '割接任务', value: `${executionMetrics.cutover.completed} / ${executionMetrics.cutover.total}` }, { label: '总完成率', value: `${totalMetrics ? Math.round(finishedMetrics / totalMetrics * 100) : 0}%` }],
    validation: [{ label: '进入验证', value: `${validationTasks.length} 台` }, { label: '配置一致', value: `${validationTasks.filter((t) => t.comparison === '一致').length} 台` }, { label: '人工确认', value: `${confirmedValidationCount} 台` }, { label: '完成批次', value: `${completedBatchIds.length} 批` }],
  })[activeStage];
  const artifacts = [
  ...
  return (
    <main className={`workspace ${navOpen ? 'nav-open' : ''} ${showInspector ? '' : 'inspector-collapsed'} ${isManagement ? 'management-view' : ''}`}>
          ...
          <div className={`conversation-content ${panel ? 'has-inline-panel' : ''}`}>
            ...
            {project && assessmentStatus !== 'completed' && (
              <div className="upload-card">
                <div className="upload-card-copy"><small>ASSESSMENT INPUTS</small><h3>上传调研评估资料</h3><p>支持 XLSX、XLS、CSV，可使用示例资料。</p></div>
            ...
            {project && activeStage === 'migration' && batchConfirmation === 'confirmed' && mdStatus !== 'ready' && <div className="md-onboarding-card">
              <div className="md-onboarding-head"><span>MD</span><div><small>MIGRATIONDIRECTOR CONNECTION</small><h3>配置近端 MigrationDirector</h3><p>在近端 MD 填写以下连接信息，然后检查配置。</p></div><em className={`md-state state-${mdStatus}`}>{mdStatusText[mdStatus]}</em></div>
      ...
      {showInspector && <AgentPanel status={currentStatus} steps={currentSteps} stats={stageStats} artifacts={artifacts} events={messages.filter((m) => m.role === 'system').map((m) => ({ text: m.text, time: m.time }))} onClose={() => { setInspectorOpen(false); setCompactInspectorOpen(false); }} />}
...
function ValidationPanel({ tasks, completedBatchIds, onConfirm, onConfirmBatch, onClose }: { tasks: ValidationVm[]; completedBatchIds: string[]; onConfirm: (id: string) => void; onConfirmBatch: (ids: string[]) => void; onClose: () => void }) {
  ...
  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel validation-panel" role="region" aria-label="今日虚拟机验证列表">
      ...
      <div className="validation-kpis"><div><small>今日进入验证</small><strong>{tasks.length}</strong><em>已完成割接的虚拟机</em></div><div><small>Agent 对比一致</small><strong>{tasks.filter((task) => task.comparison === '一致').length}</strong><em>配置自动验证通过</em></div><div><small>人工已确认</small><strong>{confirmedCount}</strong><em>{tasks.length - confirmedCount} 台待确认</em></div><div><small>迁移完成批次</small><strong>{completedBatchIds.length}</strong><em>全批次验证完成</em></div></div>
...
function CutoverTaskPanel({ tasks, onComplete, onClose, onNotify }: { tasks: VmTask[]; onComplete: (taskIds: string[]) => void; onClose: () => void; onNotify: (message: string) => void }) {
  ...
  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel task-panel cutover-panel" role="region" aria-label="今日待割接任务">
      ...
      <div className="cutover-kpis"><div><small>待割接任务</small><strong>{tasks.length}</strong><em>来自今日割接验证批次</em></div><div><small>同步已完成</small><strong>{tasks.filter((task) => task.status === '成功').length}</strong><em>可进入割接前检查</em></div><div><small>同步处理中</small><strong>{tasks.filter((task) => task.status === '同步中').length}</strong><em>持续监控数据状态</em></div><div><small>已选择</small><strong>{selectedIds.length}</strong><em>支持批量发起割接</em></div></div>
...
function SyncTaskPanel({ batches, total, completed, onClose }: { batches: BatchTask[]; total: number; completed: number; onClose: () => void }) {
  ...
  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel task-panel sync-panel" role="region" aria-label="今日待增量同步任务">
      ...
      <div className="cutover-kpis"><div><small>同步任务</small><strong>{rows.length}</strong><em>来自迁移批次计划</em></div><div><small>已完成</small><strong>{Math.min(completed, rows.length)}</strong><em>数据校验通过</em></div><div><small>运行中</small><strong>{completed < rows.length && completed > 0 ? 1 : 0}</strong><em>持续同步变化块</em></div><div><small>待执行</small><strong>{Math.max(0, rows.length - completed)}</strong><em>等待同步窗口</em></div></div>
...
function CreationTaskPanel({ tasks, onChange, onClose, onNotify }: { tasks: CreationTask[]; onChange: (tasks: CreationTask[]) => void; onClose: () => void; onNotify: (message: string) => void }) {
  ...
  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel creation-panel" role="region" aria-label="待新建迁移任务">
      ...
      <div className="creation-summary"><div><small>待新建</small><strong>{tasks.filter((task) => task.status === '待配置').length}</strong></div><div><small>已确认</small><strong>{tasks.filter((task) => task.status === '已确认').length}</strong></div><div><small>已创建</small><strong>{tasks.filter((task) => task.status === '已创建').length}</strong></div><span><i />源端检查通过，任务信息来自今日迁移批次</span></div>
...
function TaskPanel({ projectId, batches, creationTasks, risks, completedBatchIds, projectExists, onClose, onNotify }: { projectId: string; batches: BatchTask[]; creationTasks: CreationTask[]; risks: RiskItem[]; completedBatchIds: string[]; projectExists: boolean; onClose: () => void; onNotify: (message: string) => void }) {
  ...
  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel task-panel" role="region" aria-label="任务管理">
      ...
      {activeTab === 'batches' ? <>
        <div className="panel-kpis"><div><small>批次任务</small><strong>{batches.length}</strong><em>{projectExists ? '规划子智能体生成' : '等待规划设计'}</em></div><div><small>虚拟机范围</small><strong>{totalVms}</strong><em>覆盖全部规划范围</em></div><div><small>批次阶段</small><strong>{batches.length ? 3 : 0}</strong><em>试点 · 攻坚 · 扩展</em></div></div>
