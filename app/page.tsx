  ...
  return <main className="project-setup-page">
    ...
    <form className="project-setup" onSubmit={submit}>
      ...
      <div className="project-form-grid">
        ...
        <label><span>{t("行业")}</span><select value={draft.industry} onChange={(e) => setDraft({ ...draft, industry: e.target.value })}>{['金融', '运营商', '公安', '政府', '教育'].map((item) => <option key={item} value={item}>{t(item)}</option>)}</select></label>
        <label><span>{t("迁移类型")}</span><select value={draft.migrationType} onChange={(e) => setDraft({ ...draft, migrationType: e.target.value })}>{['虚拟化', 'SAN', 'NAS', '对象'].map((item) => <option key={item} value={item}>{t(item)}</option>)}</select></label>
        <label><span>{t("地区部")}</span><select value={draft.region} onChange={(e) => setDraft({ ...draft, region: e.target.value })}>{['中国地区部', '中亚地区部', '亚太地区部'].map((item) => <option key={item} value={item}>{t(item)}</option>)}</select></label>
...
function ProjectWorkspace({ onOpenSettings, navCollapsed, onNavCollapsedChange, visible, project, projects, projectId, onSelectProject, onNewProject }: { onOpenSettings: () => void; navCollapsed: boolean; onNavCollapsedChange: (collapsed: boolean) => void; visible: boolean; project: ProjectInfo | null; projects: ProjectEntry[]; projectId: string; onSelectProject: (id: string) => void; onNewProject: () => void }) {
  ...
  function sendPrompt(text: string) {
    ...
    window.setTimeout(() => {
      let answer = '';
      if (temporaryChat) answer = `这条讨论会保留在当前临时对话中。${project ? t('当前关联项目是「{0}」。', projectName) : '你可以先梳理迁移需求，准备好后再新建项目。'}\n\n${/风险|risk/i.test(text) ? '建议先确认业务停机窗口、源端与目标端兼容性，以及可验证的回退方案。' : '可以先补充源端平台、资产数量和业务约束，便于明确接下来的准备工作。'}${project ? '需要推进交付时，可切回左侧阶段会话。' : ''}`;
  ...
  return (
    <main className={`workspace ${navOpen ? 'nav-open' : ''} ${navCollapsed ? 'nav-collapsed' : ''} ${showInspector ? '' : 'inspector-collapsed'} ${isManagement ? 'management-view' : ''}`}>
      ...
      <section className="conversation-workspace">
        ...
        {!isManagement && (project || temporaryChat) && <div className="conversation-toolbar"><div className="conversation-context"><Icon name={temporaryChat ? 'chat' : 'agent'} size={15} /><span>{temporaryChat ? temporaryChats.find((chat) => chat.id === temporaryChat)?.title : project ? (currentStageChat.title === active.title ? t(active.title) : `${t(active.title)} / ${conversationTitle(currentStageChat)}`) : ''}</span></div>{temporaryChat && <span className="conversation-context-note">{t("独立讨论")}</span>}{project && !temporaryChat && !showInspector && <button className="show-execution-details" onClick={toggleInspector}><Icon name="tasks" size={14} />{t("查看执行详情")}</button>}</div>}
        <div className="conversation-scroll" ref={conversation} id={`conversation-${projectId}`} tabIndex={-1}>
          <div className={`conversation-content ${panel ? 'has-inline-panel' : ''}`}>
            ...
            {agentTyping && !isManagement && <div className="thinking-status" role="status"><span className="thinking-dot" />{t("正在整理信息…")}</div>}
  ...
  if (editingTask && draftTask) return <div className="panel-backdrop">
    <aside className="archive-panel creation-panel creation-wizard" role="region" aria-label={t("创建迁移任务")}>
      ...
      <div className="wizard-body">
        {step === 1 && <div className="wizard-section"><div className="wizard-section-head"><small>STEP 1</small><h3>{t("确认源端虚拟机与任务信息")}</h3><p>{t("源端检查已通过，可修改任务名称后进入目的端配置。")}</p></div><div className="source-summary"><div><small>{t("源端主机")}</small><strong>{draftTask.hostName}</strong></div><div><small>{t("虚拟机")}</small><strong>{draftTask.vmName}</strong></div><div><small>{t("操作系统")}</small><strong>{t(draftTask.os)}</strong></div><div><small>{t("检查结果")}</small><strong className="check-pass">{t("✓ 通过")}</strong></div></div><div className="wizard-form"><label className="wide"><span>{t("任务名称")}</span><input value={draftTask.taskName} onChange={(event) => setDraftTask({ ...draftTask, taskName: event.target.value })} /></label><label><span>{t("固件版本")}</span><select value={draftTask.firmware} onChange={(event) => setDraftTask({ ...draftTask, firmware: event.target.value as CreationTask['firmware'] })}><option value={"BIOS"}>BIOS</option><option value={"UEFI"}>UEFI</option></select></label><label><span>{t("迁移策略")}</span><select defaultValue="按计划执行"><option value={"按计划执行"}>{t("按计划执行")}</option><option value={"立即执行"}>{t("立即执行")}</option></select></label></div></div>}
        {step === 2 && <div className="wizard-section"><div className="wizard-section-head"><small>STEP 2</small><h3>{t("目的 VM 与网络配置")}</h3><p>{t("确认目标计算资源、规格、磁盘和端口组映射。")}</p></div><div className="resource-hints"><span>{t("推荐规格")}</span><b>{t(`CPU：${draftTask.cpu} 核`)}</b><b>{t(`内存：${draftTask.memory}`)}</b></div><div className="wizard-form"><label className="wide"><span>{t("目的虚拟机名称")}</span><input value={draftTask.vmName} onChange={(event) => setDraftTask({ ...draftTask, vmName: event.target.value })} /></label><label className="wide"><span>{t("计算资源")}</span><select value={computeResource} onChange={(event) => setComputeResource(event.target.value)}><option value={"FusionCompute-生产集群-A"}>{t("FusionCompute-生产集群-A")}</option><option value={"FusionCompute-生产集群-B"}>{t("FusionCompute-生产集群-B")}</option></select></label><label className="wide"><span>{t("操作系统版本")}</span><input value={draftTask.os} onChange={(event) => setDraftTask({ ...draftTask, os: event.target.value })} /></label><label><span>{t("CPU 核数")}</span><input type="number" min="1" value={draftTask.cpu} onChange={(event) => setDraftTask({ ...draftTask, cpu: Number(event.target.value) })} /></label><label><span>{t("内存大小")}</span><input value={draftTask.memory} onChange={(event) => setDraftTask({ ...draftTask, memory: event.target.value })} /></label><label><span>{t("显卡")}</span><select defaultValue="VGA · 4MB"><option value={"VGA · 4MB"}>VGA · 4MB</option><option value={"VGA · 8MB"}>VGA · 8MB</option></select></label><label className="wide"><span>{t("网卡迁移 / 端口组")}</span><select value={portGroup} onChange={(event) => setPortGroup(event.target.value)}><option value={"managePortgroup"}>managePortgroup</option><option value={"business-prod-vlan"}>business-prod-vlan</option><option value={"database-backend-vlan"}>database-backend-vlan</option></select></label><label className="wide"><span>{t("磁盘配置")}</span><input value={draftTask.disk} onChange={(event) => setDraftTask({ ...draftTask, disk: event.target.value })} /></label></div><div className="advanced-options"><label><button type="button" className={delayAllocation ? 'on' : ''} onClick={() => setDelayAllocation((value) => !value)}><i /></button><span><strong>{t("延迟分配")}</strong><small>{t("按需分配目标存储空间")}</small></span></label><label><button type="button" className={keepUuid ? 'on' : ''} onClick={() => setKeepUuid((value) => !value)}><i /></button><span><strong>{t("UUID 与源端保持一致")}</strong><small>{t("保留源端虚拟机唯一标识")}</small></span></label></div></div>}
  ...
  return <div className="panel-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="archive-panel task-panel" role="region" aria-label={t("任务管理")}>
      ...
      {activeTab === 'batches' ? <>
        ...
        <div className="migration-task-context"><div><small>{t("当前批次")}</small><select value={activeBatch?.id ?? ''} onChange={(event) => { setSelectedBatch(batches.find((batch) => batch.id === event.target.value) ?? null); setQuery(''); setStatusFilter('全部'); setSelectedVmIds([]); }} disabled={!batches.length}>{batches.length ? batches.map((batch) => <option key={batch.id} value={batch.id}>{t(batch.id)} · {t(batch.batchPhase)} · {t(batch.stageType)}</option>) : <option value={"暂无迁移批次"}>{t("暂无迁移批次")}</option>}</select></div><p><span><i className="created" />{t(`已创建${createdCount}`)}</span><span><i />{t(`未创建${Math.max(0, vmTasks.length - createdCount)}`)}</span></p></div>
