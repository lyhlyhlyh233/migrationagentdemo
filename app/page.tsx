      ...
      <aside className="workspace-nav" id={`project-navigation-${projectId}`} aria-label="项目导航">
        ...
        <div className="nav-tree-scroll">
          <nav className="primary-nav project-resources" aria-label="项目资料">
            {([{ id: 'tasks', label: '迁移任务', icon: 'tasks' }, { id: 'risk', label: '迁移风险', icon: 'shield' }, { id: 'deliverables', label: '迁移交付件', icon: 'file' }, { id: 'logs', label: '操作日志', icon: 'clock' }] as const).map((item) => <button key={item.id} className={panel === item.id ? 'selected' : ''} disabled={!project} title={!project ? `${item.label} · 请先新建项目` : undefined} onClick={() => setPanel(item.id)}><Icon name={item.icon} size={17} /><span>{item.label}</span>{item.id === 'risk' && risks.some((risk) => !risk.closed) && <span className="nav-count">{risks.filter((risk) => !risk.closed).length}</span>}</button>)}
          </nav>
