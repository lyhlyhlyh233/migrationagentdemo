...
export function SettingsDrawer({ open, onClose, onSignOut }: { open: boolean; onClose: () => void; onSignOut: () => void }) {
  ...
  const [configuration, setConfiguration] = useState<NexentConfiguration | null>(null);
  ...
  return <dialog ref={dialog} className="settings-modal" aria-labelledby="settings-title" onClose={onClose} onCancel={(event) => { event.preventDefault(); onClose(); }} onKeyDown={(event) => {
    ...
    <div className="settings-drawer">
      ...
      <div className="settings-tabs" role="tablist" aria-label={t('设置分类')} onKeyDown={(event) => {
        if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return;
        event.preventDefault();
        const next = event.key === 'Home' ? 'general' : event.key === 'End' ? 'account' : tab === 'general' ? 'account' : 'general';
        setTab(next);
        event.currentTarget.querySelector<HTMLButtonElement>(`#settings-tab-${next}`)?.focus();
        if (body.current) body.current.scrollTop = 0;
      }}>{(['general', 'account'] as const).map((item) => <button key={item} id={`settings-tab-${item}`} role="tab" aria-controls={`settings-panel-${item}`} aria-selected={tab === item} tabIndex={tab === item ? 0 : -1} onClick={() => { setTab(item); if (body.current) body.current.scrollTop = 0; }}>{t(item === 'general' ? '通用设置' : '账户与认证')}</button>)}</div>
      ...
      <footer className="settings-footer"><Icon name={tab === 'general' ? 'check' : 'info'} size={14} /><span>{t(tab === 'general' ? '偏好自动保存在此浏览器' : '认证配置需手动保存，仅在本次页面有效')}</span></footer>
    </div>
  </dialog>;
}
