...
The left header contains a native project switcher and icon actions for new project and new chat. The top progress rail is the only stage switcher and highlights the viewed stage separately from workflow progress. The sidebar lists only the most recently viewed stage's conversations, then project resources and temporary chats. Its heading pairs the stage name with a plus action. A fixed main conversation comes first; child conversations follow in creation order with inline rename controls. Stage switching restores the last conversation visited within that stage, defaulting to its main conversation.

Main conversations retain guided forms. A child starts with short project context; choosing a shortcut reveals the shared operation inside that conversation. Messages, composer drafts, pending replies and expanded panels belong to a conversation. Execution and project records are shared. Background results return to the initiating conversation and announce newly available stages without changing the current view. Management and temporary chat pages preserve sidebar stage context while hiding the rail. New project remains a standalone form, followed by the research main conversation; the empty lobby retains the disabled rail. Session state is isolated per project and resets on refresh.
