...
`WorkspaceSidePanel`（工作区非模态侧面板）提供执行详情和迁移风险两个页签；风险页签与 `RiskPanel`（独立管理页）共用 `RiskWorkspace`。同一工作区内页签切换、折叠使用隐藏属性保留已挂载内容，切换项目或会话则重新建立当前面板，不缓存所有项目页面。面板通过原生 Pointer Capture 支持拖动及键盘调宽。`Workspace` 保存页面内宽度和当前会话的页签选择，CSS 约束两侧最小宽度；这些状态不进入业务快照。评估完成时读取现有快照，首次返回该项目评估会话即自动展开风险；不增加服务命令、API 或重复回复。
